    ...
    public override bool[,] PossibleMove(TutorialChessFigure[,] ChessFigurePositions)
    {
        ...
        if (isWhite)
        {
            ...
            if (CurrentY == 4 && CurrentX > 0)
            {
                ...
                Vector2Int[] lastMove = TutorialManager.moveList[TutorialManager.moveList.Count - 1];
                if (c != null && !c.isWhite && c.GetType() == typeof(TutorialPawn))
                {
